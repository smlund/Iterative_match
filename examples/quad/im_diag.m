...
PlotEnvelope = 
Plot[ {rx[s]*1000, ry[s]*1000}, {s,0,lperiod},
  Axes             -> False, 
  Frame            -> True,
  PlotRangePadding -> {None,Automatic},
  PlotStyle        -> {Black,Red}, 
  FrameLabel       -> {"s [m]","\!\( r\_x \), \!\( r\_y \) [mm]",
                       "Matched Envelope Functions (black = x, red = y)",None},
  ImageSize        -> plotwidth, 
  BaseStyle        -> basestyle 
    ];
...
PlotEnvelopeAngle = 
Plot[ {rxp[s]*1000, ryp[s]*1000}, {s,0,lperiod},
  Axes             -> False, 
  Frame            -> True,
  PlotRangePadding -> {None,Automatic},
  PlotStyle        -> {Black,Red}, 
  FrameLabel       -> {"s [m]","\!\( r\_x\^' \), \!\( r\_y\^' \) [mrad]",
                       "Matched Envelope Angles (black = x, red = y)",None},
  ImageSize        -> plotwidth, 
  BaseStyle        -> basestyle  
    ];
...
headingmatchenv = {
{"Radii, \!\( r\_x = 2\[LeftAngleBracket] x\^2 \[RightAngleBracket]\^\(1/2\),
...
(* else, Continuous Focusing Case *)
outputmatchenv = {
 {" ", " "},
 {NumberForm[rxbar*1000,ndigits],NumberForm[rybar*1000,ndigits]},
 {NumberForm[rxmax*1000,ndigits],NumberForm[rymax*1000,ndigits]},
 {"Arb","Arb"},
 {NumberForm[rxmin*1000,ndigits],NumberForm[rymin*100,ndigits]},
 {"Arb","Arb"},
 {" ", " "},
 {NumberForm[rxpmax*1000,ndigits],NumberForm[rypmax*1000,ndigits]},
 {"Arb","Arb"},
